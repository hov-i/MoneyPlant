...
const CategoryIncomeInput = ({
  categoryIncomeList,
  categoryIncomeId,
  onCategoryIncomeIdChange,
}) => {
  const { isMobile } = useViewport();
const [activeCategoryIncomeId, setActiveCategoryIncomeId] = useState(categoryIncomeId ? categoryIncomeId : 1 ); // 초기값으로 categoryId 1 설정

  const handleIconClick = (categoryIncomeId) => {
    const category = categoryIncomeList.find(
      (item) => item.categoryIncomeId === categoryIncomeId
    );
    if (category) {
